...
class SendAPIJob implements ShouldQueue
{
    ...
    // Helper function to update API fields
    function updateApiFields($model, $body, $json) {
        if ($model) {
            $model->api_sent = 1;
            // Concatenate api_response if it already exists
            $existingResponse = $model->api_response ? json_decode($model->api_response, true) : [];
            $newResponse = json_decode($body, true);
            $model->api_response = json_encode(array_merge($existingResponse, $newResponse));

            // Concatenate api_data if it already exists
            $existingData = $model->api_data ? json_decode($model->api_data, true) : [];
            $newData = $json;
            $model->api_data = json_encode(array_merge($existingData, $newData));
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            // Fetch models
            $attendee = Attendee::where('unique_id', $this->input['unique_id'])->first();
            $stallManning = StallManning::where('unique_id', $this->input['unique_id'])->first();
            $complimentaryDelegate = ComplimentaryDelegate::where('unique_id', $this->input['unique_id'])->first();

            // Update models
            $this->updateApiFields($attendee, $body, $json);
            $this->updateApiFields($stallManning, $body, $json);
            $this->updateApiFields($complimentaryDelegate, $body, $json);
