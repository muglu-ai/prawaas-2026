...
@section('title', 'Lead Retrieval List')
            ...
            <div class="d-flex align-items-center">
                ...
                <script>
                    ...
                    document.getElementById('exportDataBtn').addEventListener('click', function(e) {
                        ...
                        let url = "{{ route('export.lead_retrieval') }}";
                        if (status) {
                                        ...
                                        @foreach ($orders as $order)
                                            ...
                                            <tr>
                                                <td class="text-dark text-md"
                                                    ...
                                                    {{ $billingDetails->name ?? $order->user->name ?? 'N/A'}}
                                                ...
                                                <td class="text-dark text-md"
                                                    style="word-break: break-word; overflow-wrap: break-word; white-space: normal; max-width: 220px;">
                                                    <span style="word-break: break-word; overflow-wrap: break-word; white-space: normal; display: block;">{{ $company ?? 'N/A' }}</span>
                                                </td>
                                                <td class="text-dark text-md"
                                                    style="word-break: break-word; overflow-wrap: break-word; white-space: normal; max-width: 220px;">
                                                ...
                                                <td class="text-dark text-md">
                                                    @if ($payments->isNotEmpty())
                                                        ...
                                                        <a href="{{ route('invoice.show', ['id' => $order->invoice->invoice_no]) }}"
                                                            ...
                                                            @php
                                                            $order->payment_verification_status = $paymentVerificationStatus ?? 'pending'; // Default to 'pending' if not set
                                                            @endphp
                                                            <br>

                                                            @if ($order->payment_verification_status == 'rejected')
                                                                <span class="text-danger">Payment Rejected</span>
                                                            @elseif ($order->payment_verification_status == 'pending')
                                                                <span class="text-warning">Payment Pending</span>
                                                            @elseif ($order->payment_verification_status == 'verified')
                                               ...
                                               @php
                                                   $leadUserFilled = \App\Models\LeadRetrievalUser::where('user_id', $order->user_id)->exists();
                                               @endphp
                                               <td class="text-dark text-md" style="word-break: break-word; overflow-wrap: break-word; white-space: normal; max-width: 220px;">
                                                   @if ($leadUserFilled)
