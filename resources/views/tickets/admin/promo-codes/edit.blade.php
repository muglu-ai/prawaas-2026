...
@section('title', 'Edit Promocode - ' . $event->event_name)
...
<div class="promo-form-container">
    <div class="promo-form-card">
        ...
        <div class="promo-form-body">
            ...
            <form action="{{ route('admin.tickets.events.promo-codes.update', [$event->id, $promoCode->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-section">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="code" class="form-control" value="{{ old('code', $promoCode->code) }}" required 
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="organization_name" class="form-control" value="{{ old('organization_name', $promoCode->organization_name) }}" 
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="value" class="form-control" value="{{ old('value', $promoCode->value) }}" 
                                   step="0.01" min="0" required placeholder="e.g., 10 or 500">
                            <div class="help-text" id="value-help">
                                @if(old('type', $promoCode->type) == 'percentage')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="valid_from" class="form-control" 
                                   value="{{ old('valid_from', $promoCode->valid_from ? \Carbon\Carbon::parse($promoCode->valid_from)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="valid_to" class="form-control" 
                                   value="{{ old('valid_to', $promoCode->valid_to ? \Carbon\Carbon::parse($promoCode->valid_to)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="unlimited_uses" id="unlimited_uses" 
                                       {{ old('unlimited_uses', $promoCode->max_uses === null) ? 'checked' : '' }}>
                                <label class="form-check-label" for="unlimited_uses">Unlimited</label>
                            </div>
                            <input type="number" name="max_uses" class="form-control" value="{{ old('max_uses', $promoCode->max_uses) }}" 
                                   min="1" id="max_uses_input" placeholder="e.g., 100" 
                                   {{ $promoCode->max_uses === null ? 'disabled' : '' }}>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Max Uses Per Contact</label>
                            <input type="number" name="max_uses_per_contact" class="form-control" 
                                   value="{{ old('max_uses_per_contact', $promoCode->max_uses_per_contact) }}" 
                ...
                <div class="form-section">
                            ...
                            <select name="applicable_registration_category_ids[]" class="form-select" multiple size="4">
                                @foreach($registrationCategories as $category)
                                    <option value="{{ $category->id }}" 
                                            {{ in_array($category->id, old('applicable_registration_category_ids', $promoCode->applicable_registration_category_ids_json ?? [])) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="applicable_ticket_category_ids[]" class="form-select" multiple size="4">
                                @foreach($ticketCategories as $category)
                                    <option value="{{ $category->id }}" 
                                            {{ in_array($category->id, old('applicable_ticket_category_ids', $promoCode->applicable_ticket_category_ids_json ?? [])) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="applicable_event_day_ids[]" class="form-select" multiple size="4">
                                @foreach($eventDays as $day)
                                    <option value="{{ $day->id }}" 
                                            {{ in_array($day->id, old('applicable_event_day_ids', $promoCode->applicable_event_day_ids_json ?? [])) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <input type="number" name="min_delegates" class="form-control" 
                                           value="{{ old('min_delegates', $promoCode->min_delegates ?? 1) }}" 
                                           min="1" placeholder="Min">
                                </div>
                                <div class="col-6">
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name="unlimited_delegates" id="unlimited_delegates" 
                                               {{ old('unlimited_delegates', $promoCode->max_delegates === null) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="unlimited_delegates">Unlimited Max</label>
                                    </div>
                                    <input type="number" name="max_delegates" class="form-control" 
                                           value="{{ old('max_delegates', $promoCode->max_delegates) }}" 
                                           min="1" id="max_delegates_input" placeholder="Max"
                                           {{ $promoCode->max_delegates === null ? 'disabled' : '' }}>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control" rows="3" 
                                  placeholder="Optional notes about this promocode...">{{ old('description', $promoCode->description) }}</textarea>
                    </div>
                </div>
