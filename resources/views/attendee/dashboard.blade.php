...
<div class="container mx-auto px-4 py-6">
    ...
    {{-- Chart: Job Category Breakdown --}}
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-info bg-gradient text-white">
            <h5 class="card-title mb-0">Job Category Breakdown</h5>
        </div>
        <div class="card-body">
            <div id="jobCategoryChart" style="height: 400px;"></div>
        </div>
    </div>

    {{-- Chart: Job Subcategory Breakdown --}}
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-warning bg-gradient text-white">
            <h5 class="card-title mb-0">Job Subcategory Breakdown</h5>
        </div>
        <div class="card-body">
            <div id="jobSubcategoryChart" style="height: 400px;"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    const jobCategoryLabels = @json($chartData['jobCategories']);
    const jobCategoryCounts = @json($chartData['jobCategoryData']);
    const jobSubcategoryLabels = @json($chartData['jobSubcategories']);
    const jobSubcategoryCounts = @json($chartData['jobSubcategoryData']);
    ...
    // Job Category Breakdown Chart
    Highcharts.chart('jobCategoryChart', {
        chart: {
            type: 'column'
        },
        title: {
        ...
        tooltip: {
            pointFormat: '<b>{point.y}</b> attendees'
        },
        series: [{
            name: 'Attendees',
            data: jobCategoryCounts,
            showInLegend: false
        }],
        credits: {
            enabled: false
    ...
    // Job Subcategory Breakdown Chart
    Highcharts.chart('jobSubcategoryChart', {
        chart: {
            type: 'bar'
        },
        title: {
        ...
        xAxis: {
            categories: jobSubcategoryLabels,
            title: {
                text: 'Job Subcategories'
            }
        },
        yAxis: {
            title: {
