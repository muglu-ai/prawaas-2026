        ...
        function verifyPayment(paymentId, status, user) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/verify-payment',  // Update route if necessary
                        type: 'POST',
                        data: {
...
{{--                                    <td>{{ $payment->currency }}</td>--}}
                                    <td>
                                        @if($payment->receipt_image)
                                            <button class="btn btn-primary" onclick="openModal({{ $payment->id }})">View</button>
                                        @else
                                                ...
                                                <div class="mt-3 input-group input-group-dynamic">
                                                   <textarea id="remarks{{ $payment->id }}" name="remarks" class="form-control" placeholder="Enter remarks for payment {{ $payment->id }}" required></textarea>
                                                    ...
                                                    <div class="mt-3 text-center">
                                                        <button type="button" class="btn btn-success" onclick="verifyPayment({{ $payment->id }}, 'success', '{{ Auth::user()->name }}')">Verify</button>
                                                        <button type="button" class="btn btn-danger" onclick="verifyPayment({{ $payment->id }}, 'reject','{{ Auth::user()->name }}')">Reject</button>
                                                    </div>
                                            </div>
                                        </div>
                                    </div>
