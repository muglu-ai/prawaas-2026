                ...
                <div class="col-md-12 mt-3">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input startup-radio" type="radio"
                            name="attendees[{{ $index }}][startup]"
                            id="startup_yes_{{ $index }}" value="1"
                            {{ (isset($data['startup']) && $data['startup'] == 1) ? 'checked' : '' }} required>
                        <label class="form-check-label" for="startup_yes_{{ $index }}">Yes</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input startup-radio" type="radio"
                            name="attendees[{{ $index }}][startup]"
                            id="startup_no_{{ $index }}" value="0"
                            {{ (isset($data['startup']) && $data['startup'] == 0) ? 'checked' : '' }} required>
                    ...
                    <div class="row">
                        @foreach ($sectors as $i => $sector)
                            @if (!empty($sector['name']))
                                <div class="col-md-4">
                                    <div class="form-check">
                                        <input class="form-check-input business-nature-checkbox" type="checkbox"
                                            id="business_nature_{{ $index }}_{{ $i }}"
                                            name="attendees[{{ $index }}][business_nature][]"
                                            value="{{ $sector['name'] }}"
                                            {{ !empty($data['business_nature']) && in_array($sector['name'], (array)$data['business_nature']) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="business_nature_{{ $index }}_{{ $i }}">
                                            {{ $sector['name'] }}
                                        </label>
                                    </div>
                                </div>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        function validateBusinessNatureRequired(idx, selected) {
                            const checkboxes = document.querySelectorAll(`[name="attendees[${idx}][business_nature][]"]`);
                            ...
                            if (selected === 'Exhibitor' || selected === 'Industry') {
                                if (form) {
                                    ...
                                    checkboxes.forEach(cb => {
                                        cb.addEventListener('change', () => {
                                            // No custom validity needed, just for user experience
                                        });
                                    });
                                }
                            } else {
