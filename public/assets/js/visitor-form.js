...
function initIntlTelInput() {
    document.querySelectorAll(".phone-input").forEach((input) => {
        ...
        // Ensure E.164 format (with country code) is set on submit
        const form = input.closest('form');
        if (form) {
            form.addEventListener('submit', () => {
            // Always set to E.164, even if not valid, to ensure country code is included
            let number = iti.getNumber().replace(/\s+/g, ''); // Remove all spaces
            input.value = number || input.value.replace(/\s+/g, '');
            console.log(`Submitting phone number for attendee: ${input.value}`);
            });
        }
    });
}

//write another function to make sure to pass the country code with the phone number
function ensureCountryCodeInPhoneNumbers() {
    document.querySelectorAll(".phone-input").forEach((input) => {
        const iti = input._iti;
        if (iti) {
            let number = iti.getNumber().replace(/\s+/g, '');
            if (!number.startsWith('+')) {
                // If not in E.164, prepend country code
...
function initSubcategoryHandlers() {
    const subCategories = {
        ...
        'Academic': ['Students', 'Teachers', 'Professor','Chips to Startup(C2S)', 'Others'],
        'Others': ['Others']
